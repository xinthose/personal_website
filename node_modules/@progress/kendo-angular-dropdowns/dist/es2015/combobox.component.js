...
export class ComboBoxComponent {
    constructor(localization, popupService, selectionService, navigationService, disabledItemsService, _zone, cdr, renderer, wrapper, touchEnabled) {
    ...
    get activeDescendant() {
        const dataItem = this.data[this.selectionService.focused];
    ...
    attachStreams() {
        ...
        this.subs.add(merge(this.navigationService.up, this.navigationService.down, this.navigationService.home, this.navigationService.end).subscribe((event) => this.navigate(event.index)));
        ...
        }), map((event) => event.indices[0]))
            .subscribe(index => {
            const dataItem = this.data[index];
            this.change(dataItem);
        }));
        this.subs.add(this.selectionService.onSelect.pipe(filter(_ => this.isOpen), tap(_ => this._filtering = false), map((event) => this.data[event.indices[0]]))
            .subscribe(dataItem => {
    ...
    search(text, startFrom = 0) {
        ...
        if (text.length && data.length) {
            const i = data.findIndex(this.findIndexPredicate(text));
    ...
    handleEnter() {
        ...
        const focusedItemText = getter(this.data[focused], this.textField);
