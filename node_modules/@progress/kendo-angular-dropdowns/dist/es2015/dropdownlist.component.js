...
export class DropDownListComponent {
    constructor(localization, popupService, selectionService, navigationService, disabledItemsService, _zone, renderer, hostElement, cdr, touchEnabled) {
    ...
    ngOnChanges(_changes) {
        ...
        const STATE_PROPS = /(value|textField|valueField|valuePrimitive|defaultItem)/g;
    ...
    subscribeEvents() {
        ...
        this.navigationSubscription = merge(this.navigationService.up.pipe(map(event => (Object.assign({}, event, { index: this.correctNavigationIndex(event.index) })))), this.navigationService.down.pipe(map(event => (Object.assign({}, event, { index: this.correctNavigationIndex(event.index) })))), this.navigationService.left.pipe(skipWhile(() => this.filterable)), this.navigationService.right.pipe(skipWhile(() => this.filterable)), this.navigationService.home, this.navigationService.end)
    ...
    correctNavigationIndex(index) {
        const focusedItemSelected = this.selectionService.isSelected(this.selectionService.focused);
    ...
    itemFromEvent(event) {
        ...
        const dataItem = isPresent(this.data[index]) ? this.data[index] : this.currentOrDefault(index);
    ...
    findDataItem({ primitive, valueField, data, value }) {
        const result = {
            dataItem: null,
            index: -1
        };
        ...
        const index = mappedData.indexOf(prop(value));
    ...
    setState() {
        ...
        const resolved = this.findDataItem({ primitive, valueField, data, value });
