...
export class MultiSelectComponent {
    constructor(localization, popupService, selectionService, navigationService, disabledItemsService, cdr, differs, renderer, hostElement, _zone, touchEnabled) {
    ...
    change(event) {
        ...
        if (isPresent(event.removed)) {
            ...
            this.value = this.value.filter(item => prop(item) !== prop(dataItem));
            this.cdr.markForCheck();
        }
    ...
    ngDoCheck() {
        if (this.differ) {
            ...
            if (valueChanges && !this.valueChangeDetected) {
                ...
                const isValueCustom = (tag) => this.data.indexOf(tag) === -1;
                const isValueSelected = (tag) => this.selectionService.isSelected(this.data.indexOf(tag));
                const tagsAndSelectionInconsistency = this.tags.some(tag => !(isValueCustom(tag) || isValueSelected(tag)));
    ...
    removeGroupTag(dataItems) {
        const prop = this.prop(this.valueField, this.valuePrimitive);
        const indices = selectedIndices(dataItems, this.data, this.valueField);
        let valuesToRemove = [];
        indices.forEach((currentValue, index) => {
            if (isNumber(currentValue)) {
                ...
                const valueToRemove = this.value.find(item => prop(item) !== prop(dataItems[index]));
        ...
        if (valuesToRemove.length) {
            ...
            valuesToRemove.forEach((value) => {
                const filter = item => prop(item) !== value;
        ...
        this.customValueSubscription = this.customValueSubject.pipe(tap(() => {
            ...
            if (isPresent(normalizedValue)) {
                ...
                if (this.value.indexOf(newValue) === -1) {
                    ...
                    const itemIndex = this.data.indexOf(newValue);
    ...
    findIndex(text, startsWith = 0) {
        let itemText;
        text = text.toLowerCase();
        let index = this.data.slice(startsWith).findIndex(item => {
