...
var DropDownListComponent = /** @class */ (function () {
    function DropDownListComponent(localization, popupService, selectionService, navigationService, disabledItemsService, _zone, renderer, hostElement, cdr, touchEnabled) {
        ...
        this.selectionSubscription = new Subscription_1.Subscription();
        this._open = false;
    ...
    DropDownListComponent.prototype.subscribeEvents = function () {
        ...
        this.selectionSubscription.add(this.selectionService.onSelect.pipe(filter_1.filter(function (_) { return _this.isOpen; }), map_1.map(this.itemFromEvent.bind(this)))
            .subscribe(this.onSelectionChange.bind(this)));
        // item selection when the popup is closed | clicked | enter, etc.
        this.selectionSubscription.add(merge_1.merge(this.selectionService.onSelect.pipe(filter_1.filter(function (_) { return !_this.isOpen; })), this.selectionService.onChange).pipe(map_1.map(this.itemFromEvent.bind(this)), tap_1.tap(function (_) { return _this.togglePopup(false); }))
        ...
        this.navigationSubscription = merge_1.merge(this.navigationService.up.pipe(map_1.map(function (event) { return (tslib_1.__assign({}, event, { index: _this.correctNavigationIndex(event.index) })); })), this.navigationService.down.pipe(map_1.map(function (event) { return (tslib_1.__assign({}, event, { index: _this.correctNavigationIndex(event.index) })); })), this.navigationService.left.pipe(skipWhile_1.skipWhile(function () { return _this.filterable; })), this.navigationService.right.pipe(skipWhile_1.skipWhile(function () { return _this.filterable; })), this.navigationService.home, this.navigationService.end)
        ...
        this._zone.runOutsideAngular(function () {
            _this.componentBlurredSubscription =
                merge_1.merge(_this.wrapperBlurred.pipe(concatMap_1.concatMap(function () { return interval_1.interval(10).pipe(take_1.take(1), takeUntil_1.takeUntil(_this.filterFocused)); })), _this.navigationService.tab).pipe(tap_1.tap(function (event) { return event instanceof navigation_service_1.NavigationEvent && _this.focus(); }), filter_1.filter(function () { return _this.isFocused; }))
    ...
    DropDownListComponent.prototype.correctNavigationIndex = function (index) {
        ...
        return (this.data.length && util_1.isPresent(this.selectionService.focused) && !focusedItemSelected) ? this.selectionService.focused : index;
    };
    ...
    DropDownListComponent.prototype.itemFromEvent = function (event) {
        ...
        var dataItem = util_1.isPresent(this.data[index]) ? this.data[index] : this.currentOrDefault(index);
